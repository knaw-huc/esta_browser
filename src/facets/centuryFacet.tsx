    ...
    return (
        ...
        <div className="hcFacet">
            ...
            <div className="hcFacetItems">
                {!loading ? (<div>
                    {data.map((item, index) => {
                        if (item.key.slice(0,1) !== '1') {return (<div key={index} className="hcFacetItem"  onClick={() => {sendCandidate(item.key)}}><div className="checkBoxLabel"> {item.key} ({item.doc_count})</div></div>);}
                    })}
                    {data.map((item, index) => {
                        if (item.key.slice(0,1) === '1') {return (<div key={index} className="hcFacetItem"  onClick={() => {sendCandidate(item.key)}}><div className="checkBoxLabel"> {item.key} ({item.doc_count})</div></div>);}
                    })}
